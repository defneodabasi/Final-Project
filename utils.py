...
def colorBackgroundText(img, text, font, fontScale, textPos, textThickness=1,textColor=(0,255,0), bgColor=(0,0,0), pad_x=3, pad_y=3):
    ...
    img:(mat) which you want to draw text
    text: (string) text you want draw
    font: fonts face, like FONT_HERSHEY_COMPLEX, FONT_HERSHEY_PLAIN etc.
    fontScale: (double) the size of text, how big it should be.
    textPos: tuple(x,y) position where you want to draw text
    textThickness:(int) fonts weight, how bold it should be
    textPos: tuple(x,y) position where you want to draw text
    textThickness:(int) fonts weight, how bold it should be.
    textColor: tuple(BGR), values -->0 to 255 each
    bgColor: tuple(BGR), values -->0 to 255 each
    pad_x: int(pixels)  padding of in x direction
    pad_y: int(pixels) 1 to 1.0 (), controls transparency of  text background 
